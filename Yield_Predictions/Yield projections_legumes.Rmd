...
pacman::p_load(
   ...
   rnaturalearthdata, metfor, broom, broom.mix, lme4
)
```
...
```{r matafor package,echo=T, message=FALSE}
df <- dataset

# --- Clean and filter for legume crops only ---
legume_keywords <- c("bean", "pea", "lentil", "chickpea", "cowpea", "pigeonpea", "soy", "soybean", "groundnut", "gram", "legume")

df <- df %>%
  mutate(
    Crop_lower = tolower(Crop),
    is_legume = sapply(Crop_lower, function(x) any(grepl(paste(legume_keywords, collapse = "|"), x)))
  ) %>%
  filter(is_legume)
...
df_meta <- df %>%
  select(Study = Reference,
         ...
         Year = `Future_Mid-point`,
         Site = `Site(location)`,
         Model = `GCM/RCM`,
         CropModel = `Crop Model`,
         yi = `Climate impacts (%)`) %>%
  filter(!is.na(yi), !is.na(Crop), !is.na(Scenario))

# --- UNWEIGHTED META-ANALYSIS ---
# Basic pooled estimate (mean)
pooled_mean <- mean(df_meta$yi)
pooled_sd <- sd(df_meta$yi)
n <- nrow(df_meta)
tau2 <- var(df_meta$yi)
se_pooled <- sqrt(tau2 / n)
ci_lower <- pooled_mean + qnorm(0.025) * se_pooled
ci_upper <- pooled_mean + qnorm(0.975) * se_pooled

cat("Pooled Estimate (Unweighted):", round(pooled_mean, 2), "%\n")
cat("95% CI: [", round(ci_lower, 2), ",", round(ci_upper, 2), "]\n")
cat("Between-study Variance (tau²):", round(tau2, 2), "\n\n")

# --- MODERATOR ANALYSIS (OLS) ---
model <- lm(yi ~ Crop + Scenario, data = df_meta)
summary(model)

# --- OPTIONAL: Forest plot of top 50 extreme values ---
top_50 <- df_meta %>%
  slice_max(order_by = abs(yi), n = 50) %>%
  mutate(label = paste0(Study, " (", Crop, ")"))

forest_plot<-ggplot(top_50, aes(x = yi, y = reorder(label, yi))) +
  geom_point(color = "steelblue") +
  geom_vline(xintercept = 0, linetype = "dashed") +
  geom_vline(xintercept = pooled_mean, color = "red") +
  labs(
    title = "Top 50 Most Extreme Projected Yield Changes (Legumes)",
...
```{r convert to proteing factors,echo=T, message=FALSE}
# --- Define protein content per crop ---
protein_lookup <- data.frame(
  Crop = c("Soybean", "Chickpea", "Common Bean", "Cowpea", "Groundnut", 
           "Lentil", "Faba Bean", "Pigeonpea", "Green gram"),
  Protein_Content = c(0.36, 0.21, 0.23, 0.24, 0.25, 0.26, 0.27, 0.22, 0.24)
)

# --- Identify legumes, join protein factors ---
legume_keywords <- c("bean", "pea", "lentil", "chickpea", "cowpea", "pigeonpea", "soy", "soybean", "groundnut", "gram", "legume")
...
df_protein <- dataset %>%
  ...
  filter(is_legume, !is.na(`Climate impacts (%)`), !is.na(Crop), !is.na(`Climate scenario`)) %>%
...
# --- Set as factor and relevel reference ---
...
# --- Fit model using grouped adaptation types ---
model_adapt_grouped <- lm(Protein_Yield_Change ~ Adaptation_Group, data = df_protein)
summary(model_adapt_grouped)

# --- Create plot from model output ---
library(broom)
library(ggplot2)
