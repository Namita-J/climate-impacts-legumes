...
pacman::p_load(
   ...
   rnaturalearthdata, metafor, broom, broom.mixed, lme4, grid, gridExtra, patchwork, 
   emmeans
)
```
...
data_url <- "https://github.com/Namita-J/climate-impacts-legumes/raw/main/Yield_Predictions/yield_predictions_data.xlsx"
...
This vignette aims to explore the data on the projected impacts of
climate change on legume crops in Africa. It will explore the following:
...
*5. Understanding how different practices reduce the impacts of climate
change*
...
To conduct a comprehensive meta-analysis on the climate impacts on
legumes in Africa, a systematic literature search was performed using
the Web of Science (WoS) database. The search terms were designed to
capture studies that explore climate change, its various drivers, and
impacts on agricultural productivity, particularly focusing on legumes,
within the African context. The search strategy incorporated Boolean
operators, proximity operators, and specific geographical filters to
ensure relevance and precision. Below are the details of the search
terms used:

**Climate Change-Related Terms**

The following terms were used to capture studies focused on climate
change, its drivers, and related phenomena:
...
To ensure relevance to agricultural productivity and legumes, the
...
The final search string combined all the above elements using the
Boolean operator `AND`:

-   Climate-related terms AND Agriculture and food productivity terms
    AND Geographical filters.

This comprehensive search strategy was implemented to identify all
relevant studies addressing climate change impacts on agricultural
productivity, particularly legumes, in Africa.
...
This screening criteria is designed to systematically review papers
focusing on crop yield impacts, applying both inclusion and exclusion
criteria based on specific parameters. The framework ensures that the
selected studies align with the objectives of the review.

*Population (P):* The review includes studies focusing on legume crops
grown in African countries. The geographic scope is strictly limited to
Africa. Studies involving crops other than legumes are excluded.

*Intervention/Practice (I):* Eligible studies must be modeling-based and
provide future yield projections. Studies incorporating adaptation
strategies, such as reduced tillage or irrigation, are considered highly
valuable but not mandatory for inclusion.

*Comparison (C):* Studies should ideally include a control group for
comparison. However, for modeling studies, this requirement is flexible
and depends on the nature of the projections (e.g., baseline scenarios
can serve as controls).

*Outcomes (O):* The primary outcome of interest is projected yield data
for legume crops. Studies reporting projections based on different
scenarios (e.g., climate change impacts, adaptation strategies) are
prioritized.

*Study Design (S):* Only modeling studies are eligible for inclusion.
Studies must explicitly include projections for future yields, either
with or without adaptation strategies.

## Screening Outcomes

![PRISMA Diagram for
Legumes](https://raw.githubusercontent.com/Namita-J/climate-impacts-legumes/main/Yield%20Predictions/images/PRISMA%20Diagram%20legumes.png)
...
# Count number of studies by country and crop
crop_counts <- filtered_data %>%
  group_by(Crop) %>%
  summarise(N = n_distinct(Reference), .groups = "drop")

# Count total number of papers per country
paper_counts <- filtered_data %>%
  group_by(Country) %>%
  summarise(N_Papers = n_distinct(Reference), .groups = "drop")

# Convert to spatial format
filtered_sf <- st_as_sf(filtered_data, coords = c("longitude", "latitude"), crs = 4326)
...
# Join country-level paper counts to map
map_data <- world %>%
...
map <- ggplot() +
  ...
  coord_sf(xlim = c(-20, 52), ylim = c(-35, 50), expand = FALSE)

# ---- Create static crop count table ----
crop_table <- crop_counts %>%
  arrange(desc(N)) %>%
  head(20)  # adjust number of rows shown

table_grob <- tableGrob(crop_table)

# ---- Combine map and table side by side ----
...
This script analyzes the impact of climate on the yield of selected
legumes by filtering the dataset to include specific crops with
available data on climate impacts, ensuring no missing values in the
Climate impacts (%) column. The data is grouped by crop, and the number
of observations for each crop is calculated to provide context for the
analysis.

The plot reveals the distribution of climate impacts across different
crops, highlighting trends in the data.Some crops, such as Common Bean
and Groundnut, exhibit a wider range of climate impacts, with both
positive and negative extremes. In contrast, crops like Green Grams show
relatively limited variability, suggesting more consistent responses to
climate factors. However, this is also attributed the number of papers
done for each of these crops.

Additionally, certain crops, such as Cowpea and Soybean, display
clusters of outliers, indicating occasional extreme climate impacts that
...
This analysis explores how different climate scenarios impact crop
yields, combining insights from both CMIP5 (RCP scenarios) and CMIP6
(SSP scenarios). The dataset focuses on selected crops and scenarios
where reliable data on Climate impacts (%) is available, allowing for a
meaningful comparison. By including both RCPs and SSPs, the analysis
bridges older and newer climate modeling frameworks, recognizing the
ongoing relevance of CMIP5 data in climate research and its ability to
provide valuable insights alongside the more recent CMIP6 projections.

The plot reveals some clear trends. Under high-emission scenarios like
RCP8.5 and SSP585, the variability in climate impacts is much wider,
with a notable number of positive impacts, suggesting that certain crops
might benefit under these extreme warming conditions. On the other hand,
low-emission scenarios like SSP126 and RCP2.6 show more consistent
impacts, with narrower ranges and fewer extreme values. This reflects
the more moderated effects expected when strong mitigation measures are
implemented.

Despite the release of CIMP6,this study chose to retain crop yield
projections done with CIMP5 and CIMP3 as they still provide valuable
insights.
...
This script explores how different adaptation strategies help mitigate
the effects of climate change on crop yields. The analysis focuses on
selected crops with valid data on Climate impacts (%) and groups the
strategies to evaluate their effectiveness. The plot reveals that
approaches like using high fertilizer levels and selecting specific
cultivars are particularly effective at reducing negative impacts. In
contrast, strategies like late planting show greater variability and
higher risks. Combined strategies stand out for their consistent
results, offering solutions to enhance crop resilience.
...
The treatment vs. control approach greatly reduces the number of studies
included (7), as most focus on a single adaptation strategy rather than
projecting both scenarios. This script filters the dataset for selected
crops, separating "no adaptation" (control) and specific adaptation
strategies (treatment). The two groups are merged based on shared
attributes like site, crop, and climate scenario, allowing for direct
comparisons.

The plot shows yield changes across management strategies, with "no
adaptation" as the baseline. Strategies like combination approaches and
irrigation consistently improve yields, while others, such as late
planting, show mixed results. Despite limited data availability, this
analysis highlights the value of directly comparing treatment and
control to assess the real impact of adaptation strategies
...
This script automates the process of querying the OpenAlex API for
research on climate change, agriculture, and African regions. It
constructs Boolean search strings for climate, agriculture, and regional
terms, combines them into a final query, and retrieves results within a
specified date range (1970--2024). The results, including key details
like title and DOI, are saved in a CSV file for analysis.\
\
...
The majority of references have low relevance scores, as shown by the
high frequency of scores concentrated near zero. There is a rapid
decline in frequency as relevance scores increase, with very few
references scoring above 100. This pattern suggests that most of the
retrieved works have only moderate alignment with the search criteria,
while a small subset demonstrates stronger relevance to the specified
terms.\
\
...
This script compares results from two bibliographic databases, Web of
Science (WoS) and OpenAlex, to assess overlap and relevance in
identifying studies on climate change impacts on legumes. It begins by
loading accepted and rejected WoS references, cleaning and standardizing
titles and DOIs for consistent matching.

OpenAlex query results are also processed similarly, ensuring
compatibility. Matches are identified by comparing titles and DOIs
across datasets, with duplicates removed. Matched data is analyzed to
assess overlap, including creating histograms to visualize the
distribution of relevance scores for accepted and rejected references.
Additionally, the script includes functionality to fetch records from
OpenAlex using DOIs and partial title matching.
...
1.  Meta-Analysis with the metafor Package To quantify the effects of
    different climate adaptation strategies on projected legume yield,
    we used the metafor package in R --- a widely adopted tool for
    conducting meta-analyses across disciplines. The package allows for
    the estimation of pooled effect sizes while accounting for
    study-level variance and moderator effects, using both fixed- and
    random-effects models. In this analysis, we applied unweighted and
    moderator-enhanced models using rma() to assess average adaptation
    impacts and heterogeneity across scenarios and crop types.

2.  Converting Yield Projections to Protein Equivalents Projected yield
    changes were typically reported as percentage change in yield (t/ha)
    under future climate scenarios, with and without adaptation.
    However, different legumes vary significantly in their protein
    content per unit weight, which has major implications for food
    security and nutritional outcomes.

To standardize comparisons across crops, we applied a protein content
multiplier to each yield projection. For example:

Protein-Adjusted Yield Change = Projected Yield Change (%) × Protein
Content (g/g)

Each legume crop is assigned a typical protein content based on
published composition data.

3.  Justification for Combining Legume Crops Legume crops are not
    nutritionally or agronomically identical, but after adjusting for
    protein content, they can be compared on a common nutritional basis
    which is their contribution to digestible protein per hectare. This
    transformation removes bias introduced by crops that produce large
    volumes but lower nutritional density (e.g., green gram vs.
    soybean), and instead evaluates each adaptation strategy's impact on
    protein.

By converting to a standardized protein-equivalent metric, we are able
to:

-   Meaningfully pool results across diverse legume species

-   Detect general adaptation trends applicable across crop types

-   Highlight which strategies consistently increase nutritional yield,
    not just bulk production

```{r convert to protein factors,echo=T, message=FALSE}
# --- Protein content lookup (only for observed legumes) ---
protein_lookup <- data.frame(
  Crop = c("Common Bean", "Groundnut", "Cowpea", "Soybean", 
           "Bush Bean", "Climbing Bean", "Faba Bean", "Green grams"),
  Protein_Content = c(0.23, 0.25, 0.24, 0.36, 0.23, 0.23, 0.27, 0.24)
)

# --- Filter and compute protein yield change ---
df_protein <- dataset %>%
  filter(Crop %in% legumes, 
         !is.na(`Climate impacts (%)`), 
         !is.na(Crop), 
         !is.na(`Climate scenario`)) %>%
  rename(yi = `Climate impacts (%)`,
         Scenario = `Climate scenario`,
         Study = Reference) %>%
  left_join(protein_lookup, by = "Crop") %>%
  filter(!is.na(Protein_Content)) %>%
  mutate(Protein_Yield_Change = yi * Protein_Content)

# --- Recode AdaptationType into broader groups ---
df_protein <- df_protein %>%
  filter(!is.na(`Adaptation type`)) %>%
  rename(AdaptationType = `Adaptation type`) %>%
  mutate(Adaptation_Group = case_when(
    grepl("Combined", AdaptationType, ignore.case = TRUE) ~ "Combined",
    grepl("Cultivar", AdaptationType, ignore.case = TRUE) ~ "Cultivar",
    grepl("Fertilizer", AdaptationType, ignore.case = TRUE) ~ "Fertilizer",
    grepl("Irrigation", AdaptationType, ignore.case = TRUE) ~ "Irrigation",
    grepl("Planting", AdaptationType, ignore.case = TRUE) ~ "Planting date",
    AdaptationType == "No" ~ "No adaptation",
    TRUE ~ "Other"
  ))

# --- Set factor level and relevel reference (if exists) ---
df_protein$Adaptation_Group <- factor(df_protein$Adaptation_Group)
if ("No adaptation" %in% levels(df_protein$Adaptation_Group)) {
  df_protein$Adaptation_Group <- relevel(df_protein$Adaptation_Group, ref = "No adaptation")
}
```

```{r run the metafor package,echo=T, message=FALSE}
model_adapt_grouped <- lm(Protein_Yield_Change ~ Adaptation_Group, data = df_protein)

# --- Summarize and style output ---
effects <- tidy(model_adapt_grouped) %>%
  filter(term != "(Intercept)") %>%
  mutate(
    Adaptation_Type = gsub("Adaptation_Group", "", term),
    ...
    Label = paste0(sprintf("%.2f", estimate), " ", Significance),
    Color = case_when(
      p.value >= 0.05 ~ "gray",
      estimate > 0 ~ "forestgreen",
      TRUE ~ "firebrick"
    )
  )

# --- Plot ---
adapt_effects <- ggplot(effects, aes(x = estimate, y = reorder(Adaptation_Type, estimate), fill = Color)) +
  geom_bar(stat = "identity", width = 0.4) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_text(aes(label = Label, hjust = ifelse(estimate > 0, -0.1, 1.1)),
            size = 2, color = "black", fontface = "bold") +
  scale_fill_identity() +
  labs(
    title = "Effect of Adaptation Type on Protein-Adjusted Yield Change",
    x = "Estimated % Change in Protein Yield (vs No Adaptation)",
    y = NULL
  ) +
...
# --- Table display next to plot ---
table_data <- effects %>%
  select(Adaptation_Type, estimate, p.value) %>%
  rename(
...
table_grob$widths <- unit(rep(1/3, ncol(table_data)), "npc")

# --- Display plot and table side by side ---
adapt_effects + wrap_elements(table_grob) + 
  plot_layout(ncol = 2, widths = c(2.5, 1.6))
```
```{r run the metafor package with climate scenarios , eval= T, echo=T, message=FALSE}
# Fit interaction model
model_interaction <- lm(Protein_Yield_Change ~ Adaptation_Group * Scenario, data = df_protein)

# --- Clean and extract coefficients ---
interaction_effects <- tidy(model_interaction) %>%
  filter(!is.na(p.value), term != "(Intercept)") %>%
  mutate(
    # Extract Adaptation Type
    Adaptation_Type = case_when(
      grepl("Adaptation_Group", term) ~ gsub("Adaptation_Group", "", sub(":.*", "", term)),
      TRUE ~ NA_character_
    ),
    # Extract Scenario
    Scenario = case_when(
      grepl("Scenario", term) ~ gsub("Scenario", "", sub(".*:", "", term)),
      TRUE ~ "Baseline"
    ),
    Significance = case_when(
      p.value < 0.001 ~ "***",
  ...
  filter(!is.na(Adaptation_Type), Scenario != "Baseline")  # Remove NA and Baseline terms



# Plot grouped bars by scenario
interaction_plot <- ggplot(interaction_effects, aes(x = Adaptation_Type, y = estimate, fill = Scenario)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.7), width = 0.6) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_text(aes(label = Label, group = Scenario),
            position = position_dodge(width = 0.7),
            hjust = ifelse(interaction_effects$estimate > 0, -0.1, 1.1),
            size = 2.5, color = "black", fontface = "bold") +
  scale_fill_brewer(palette = "Set2") +
  labs(
    title = "Interaction: Adaptation Type × Climate Scenario",
    x = "Adaptation Type",
    y = "Estimated % Change in Protein Yield",
...
```
In our main analysis, we used the metafor package to run an unweighted meta-analysis. This was necessary because most studies didn’t report key variance measures like standard errors or confidence intervals, which are typically used to weight effect sizes. While metafor is well-suited for meta-analysis, it generally performs best when such variance data is available.

To strengthen our findings and account for the fact that multiple data points came from the same studies, we ran a sensitivity check using a mixed-effects model with the lmerTest package. This allowed us to include Study (defined by unique DOIs) as a random effect, helping to control for variation between studies.

This model tested whether the estimated effects of different adaptation types held up after adjusting for study-level clustering and it also provided p-values for the fixed effects, something that isn’t included by default in standard lme4 models.

By treating Study as a random effect, this approach helped protect against overconfidence in the results and offered an added layer of robustness to support the patterns we observed in the unweighted analysis.
```{r random-effect-model-sensitivity, message=FALSE, warning=FALSE}
...
# --- Define Study as unique DOI ---
df_protein <- df_protein %>%
  mutate(Study = doi)

# --- Ensure Adaptation_Group is a factor and relevel ---
df_protein$Adaptation_Group <- factor(df_protein$Adaptation_Group)
df_protein$Adaptation_Group <- relevel(df_protein$Adaptation_Group, ref = "No adaptation")

# --- Fit the random effects model with Study as grouping variable ---
model_random <- lmer(Protein_Yield_Change ~ Adaptation_Group + (1 | Study), data = df_protein)

# --- Extract fixed effects with p-values ---
effects_random <- tidy(model_random, effects = "fixed") %>%
  filter(term != "(Intercept)") %>%
  mutate(
    Adaptation_Type = gsub("Adaptation_Group", "", term),
    Significance = case_when(
...
# --- Plot fixed effects with significance labels ---
plot_random <- ggplot(effects_random, aes(x = estimate, y = reorder(Adaptation_Type, estimate), fill = Color)) +
...
plot_random
```
C3 crops like soybean, common bean, and groundnut use the Calvin cycle for carbon fixation, which is more sensitive to atmospheric CO2₂ levels compared to c4 crops like maize. Under elevated CO₂,c3₃ plants can photosynthesize more efficiently, leading to:

Increased carbon assimilation

Improved water use efficiency (due to reduced stomatal conductance)

Potential gains in biomass and yield

Because of this physiological responsiveness, it's especially important to assess how projected yield changes under climate scenarios differ with and without CO₂ fertilization — particularly for C₃ species. Ignoring CO₂ effects may underestimate their adaptive potential or skew comparisons with C₄ crops.


```{r effects of co2, message=FALSE, warning=FALSE}

df_co2 <- dataset %>%
  filter(Crop %in% c("Maize", "Soybean"),
         `Adaptation type` == "No",
         !is.na(`Climate impacts (%)`),
         !is.na(CO2),
         !is.na(doi)) %>%
  mutate(
    CO2 = factor(CO2, levels = c("No", "Yes")),
    Crop = factor(Crop),
    Study = doi,
    Yield_Change = `Climate impacts (%)`
  )

model_co2 <- lmer(Yield_Change ~ CO2 * Crop + (1 | Study), data = df_co2)

# Estimate marginal means for each Crop × CO2 combo
em_co2 <- emmeans(model_co2, ~ Crop * CO2)

# Convert to dataframe for plotting
df_em <- as.data.frame(em_co2) %>%
  mutate(
    Label = paste0(sprintf("%.2f", emmean)),
    Color = case_when(
      CO2 == "Yes" ~ "forestgreen",
      CO2 == "No" ~ "firebrick"
    )
...
ggplot(df_em, aes(x = emmean, y = interaction(Crop, CO2), fill = Color)) +
  geom_bar(stat = "identity", width = 0.6, show.legend = FALSE) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "black") +
  geom_text(aes(label = Label), hjust = ifelse(df_em$emmean > 0, -0.1, 1.1),
            size = 3.5, color = "black", fontface = "bold") +
  scale_fill_identity() +
  labs(
    title = "Estimated Yield Change by Crop and CO2- No adaptation",
    x = "Estimated % Change in Yield",
    y = NULL
  ) +
  theme_minimal(base_size = 8) +
  theme(
